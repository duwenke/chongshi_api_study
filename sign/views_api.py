...
def add_event(request):
    ...
    if result:
        return JsonResponse({'status':10022,'message':'事件id已存在'})

    result = Event.objects.filter(name=name)
    if result:
        return JsonResponse({'status':10023,'message':'事件名称已存在'})
...
def add_guest(request):
    ...
    result = Event.objects.filter(id=eid)
    if not result:
        return JsonResponse({'status':10022,'message':'事件id不存在'})

    result = Event.objects.get(id=eid).status
    if not result:
        return JsonResponse({'status':10023,'message':'事件状态不可用'})
    ...
    try:
        ...
        return JsonResponse({'status':10026,'message':'手机号已存在'})
